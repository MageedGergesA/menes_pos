...
class MezzeOcbDisplay(models.Model):
    ...
    def _price_lines(self, lines):
        """Money for this display's pre-fire cart.

        The algorithm itself is `mezze.cart.pricing` — ONE definition, so the
        operator's order panel and the customer's board can never disagree about
        what a cart costs. This display only supplies the branch it belongs to.
        """
        self.ensure_one()
        return self.env['mezze.cart.pricing']._price_cart(self.config_id, lines)
